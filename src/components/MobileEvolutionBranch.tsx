...
const MobileEvolutionBranch = ({ startPokemonName, evolutionPaths, evolutionArtworks, onSelect, isPending }: MobileEvolutionBranchProps) => {
      ...
      {branchPaths.length > 0 && (
        ...
        <ul className={branchPaths.length > 1 ? "mobile-evolution-branches ml-17" : undefined}>
          {branchPaths.map((path) => (
            <MobileEvolutionBranch key={path.to.name} startPokemonName={path.to.name} evolutionPaths={evolutionPaths} evolutionArtworks={evolutionArtworks} onSelect={onSelect} isPending={isPending} />
          ))}
        </ul>
      )}
    </li>
